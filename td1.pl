%TD 21/11
%Penser que l'on manipule une liste chainée

%appartient(E ,L)
%E est un element de la liste L
%?C'est la condition d'arr阾e
appartient(E, [E|_]).
%?C'est la r鑗le qui permet de faire l'it閞ation
%  Ici, Prolog essaie de trouver le cas
%  correspondant la condition d'arr阾e, en suprimant
%  pas ?pas, l'閘閙ent au d閎ut de la liste, puisque
%  on a [_] au d閎ut, qui veut dire que cet 閘閙ent n'est
%  pas n閏essaire de consid閞er.
appartient(E, [_|L]):-
	appartient(E, L).

% pourquoi ci-dessous est faux?
% en fait '_' ici signifie le premier 閘閙ent
% de la liste, et E est la queue de la liste,
% qui peut 阾re une sous liste.
...
%extraire(E,L,R)
%R=L-E
extraire(D,[D|E],E).%Pourquoi avec 鏰 on ne
...
%concateliste(LL,LR).
%LL est une liste de listes on concat鑞e
...
%TD2-1
%renverser(L1,L2)
%L2 est la lishte "renvers閑" de L1
%Version1, d閜lacer le 1er au dernier
renverser1([],[]):-!.
renverser1([E|Q], L2):-
	renverser1(Q, L1),
	append(L1, [E], L2).

%Version2, d閜lacer le dernier au permier
renverser2([],[]):-!.
renverser2(L1, [E|Q]):-
%	dernier(E, L1),
	append(X, [E], L1),
...
%Version3
%Comlexit? 鏰 me semble pas mal quand on veut
...
somme(L,R):-
	somme_3(L, 0, R).
somme_3([], S, S).%Le 3鑝e arg est seulement pour
                 %r閏up閞er le resultat.
somme_3([A|L], S, R):-
	S1 is S+A,
	somme_3(L, S1, R).
...
%Version 2, bonne, mais la consommation de m閙oire..
somme_synbo3([E], E):-!.
...
%--------------------------------------------------------------
%conterbon(L, V, R).
%L est une liste de nombres, connus, entr閑.
%V est un nombre, entr閑.
%R est une somme d'閘閙ent L, L=V, sortie.
